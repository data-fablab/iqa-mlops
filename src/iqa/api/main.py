...
from iqa.monitoring.exposition import (
    ObservabilityExposition,
...
# Lifecycle/drift observability exposition (state + sanitisation + Prometheus
# rendering) lives in ``iqa.monitoring.exposition``. The API owns one instance
# and the three routes below delegate to it.
OBSERVABILITY = ObservabilityExposition()
...
def _translate_observability_rejection(rejection: ObservabilityRejection) -> None:
    _raise_api_error(
...
def record_lifecycle_metric_event(
    ...
    _require_token("IQA_SERVICE_TOKEN", x_iqa_service_token)
    try:
        OBSERVABILITY.record_lifecycle_event(request)
    except ObservabilityRejection as rejection:
        _translate_observability_rejection(rejection)
    return {
        "accepted": True,
...
def record_drift_metric_event(
    ...
    _require_token("IQA_SERVICE_TOKEN", x_iqa_service_token)
    try:
        OBSERVABILITY.record_drift_event(request)
    except ObservabilityRejection as rejection:
        _translate_observability_rejection(rejection)
    return {
        "accepted": True,
...
def _append_counter_lines(
    ...
    for labels, value in sorted(counters.items(), key=lambda item: str(item[0])):
        lines.append(f"{metric_name}{{{metric_labels(labels)}}} {value}")
...
def metrics() -> str:
    ...
    lines.extend(OBSERVABILITY.render_prometheus_lines())
    return "\n".join(lines) + "\n"
