"""Run the IQA lifecycle decision boundary.

This is the ``lifecycle_decision`` task of ``iqa_lifecycle`` run as a container
(ADR 0008, issue 08): it evaluates the data-event trigger rule **inside the data
image** -- no ``iqa`` import in the Airflow scheduler -- and prints the decision
as JSON (the container stdout is the task's XCom: references only, no payloads).
"""
...
def main() -> None:
    ...
    result = {
        "service": "iqa-lifecycle-decision",
