...
RUN --mount=type=cache,target=/root/.cache/uv pip install --no-cache-dir uv && uv sync --no-dev
...
RUN --mount=type=cache,target=/root/.cache/uv uv sync --no-dev --extra serving

# --- ml: iqa-inference, iqa-trainer (torch + scikit-learn + mlflow) ---
FROM base AS ml
ARG IQA_TORCH_EXTRA=cpu
RUN --mount=type=cache,target=/root/.cache/uv uv sync --no-dev --extra serving --extra ml --extra "$IQA_TORCH_EXTRA"

# --- data: iqa-ingestion, iqa-replay, iqa-monitoring (no torch) ---
FROM base AS data
RUN --mount=type=cache,target=/root/.cache/uv uv sync --no-dev --extra data
...
RUN --mount=type=cache,target=/root/.cache/uv uv sync --no-dev --extra dvc
