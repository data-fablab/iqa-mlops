...
class TestStatusComputation:
    """Test status computation logic (enables refactoring of threshold logic)."""

    def test_green_status_below_orange_threshold(self) -> None:
        """Score below orange threshold produces green status."""
        from iqa.inference.helpers import compute_status

        status = compute_status(0.01, threshold_orange=0.02, threshold_red=0.05)
        assert status == "green"

    def test_orange_status_between_thresholds(self) -> None:
        """Score between orange and red thresholds produces orange status."""
        from iqa.inference.helpers import compute_status

        status = compute_status(0.03, threshold_orange=0.02, threshold_red=0.05)
        assert status == "orange"

    def test_red_status_above_red_threshold(self) -> None:
        """Score above red threshold produces red status."""
        from iqa.inference.helpers import compute_status

        status = compute_status(0.06, threshold_orange=0.02, threshold_red=0.05)
        assert status == "red"

    def test_status_at_orange_boundary(self) -> None:
        """Score at orange threshold produces orange status (inclusive)."""
        from iqa.inference.helpers import compute_status

        status = compute_status(0.02, threshold_orange=0.02, threshold_red=0.05)
        assert status == "orange"

    def test_status_at_red_boundary(self) -> None:
        """Score at red threshold produces red status (inclusive)."""
        from iqa.inference.helpers import compute_status
