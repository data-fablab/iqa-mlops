...
from scripts.calibrate_feature_ae_champion import (
    assert_validation_has_defects,
    build_champion_thresholds,
...
def test_champion_business_metric_priority_prefers_pixel_aupimo() -> None:
...
def test_champion_business_metric_falls_back_to_pixel_ap() -> None:
...
def test_champion_thresholds_expose_scoring_contract() -> None:
    ...
    assert thresholds["score_contract_version"] == CHAMPION_FEATURE_AE_CONTRACT.version
...
def test_champion_validation_requires_defective_gt_masks(tmp_path: Path) -> None:
...
def test_champion_calibration_matrix_contains_pixel_metrics(tmp_path: Path) -> None:
    path = write_calibration_matrix(
        ...
        {
            "score_contract_version": "feature_ae_champion_v001",
            "selected_metric": "pixel_aupimo_1e-5_1e-3",
            "selected_metric_value": 0.417,
...
def test_update_champion_manifest_writes_complete_contract(tmp_path: Path, monkeypatch) -> None:
    manifest_path = tmp_path / "model_manifest.json"
    manifest_path.write_text('{"model_version": "candidate"}\n', encoding="utf-8")
    monkeypatch.setattr("scripts.calibrate_feature_ae_champion.model_manifest_path", lambda _version: manifest_path)
    ...
    contract = manifest["feature_ae_champion_contract"]
    assert manifest["preprocessing_contract_version"] == "feature_ae_champion_v001"
...
def test_champion_calibration_cli_is_exposed() -> None:
    pyproject = tomllib.loads(Path("pyproject.toml").read_text(encoding="utf-8"))

    assert (
        pyproject["project"]["scripts"]["iqa-calibrate-feature-ae-champion"]
        == "scripts.calibrate_feature_ae_champion:main"
    )
