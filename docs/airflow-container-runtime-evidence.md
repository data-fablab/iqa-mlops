...
- `iqa_ingestion`, `iqa_replay`, `iqa_monitoring`, `iqa_lifecycle`,
  `iqa_lifecycle_trigger`, `iqa_drift_piece_a_p4` and
  `iqa_drift_correction_lifecycle` use `make_container_task`.
...
- the DAG list includes `iqa_ingestion`, `iqa_replay`, `iqa_monitoring`,
  `iqa_lifecycle`, `iqa_lifecycle_trigger`, `iqa_drift_piece_a_p4`,
  `iqa_drift_correction_lifecycle` and `iqa_dvc_reproducibility`;
...
- `iqa_drift_piece_a_p4` observes the Piece B -> Piece A/P4 replay before
  triggering one `iqa_drift_correction_lifecycle` run with a drift context;
